...
int setup_child_capabilities()
{
	...
	//step 1: dropping capabilities from the AMBIENT set --- use prctl()
	...
	cap_t caps = cap_get_proc();		//getting capability sets of the process 
	...
	//step 2: clear capabilities from inheritable set 	--- use cap_set_flag
	...
	//set the cleared set back to the process --- use cap_set_proc
...
int setup_syscall_filters()
{
	...
	//1. setting up filtering context with default behavior
	scmp_filter_ctx seccomp_ctx = seccomp_init(SCMP_ACT_ALLOW);
	if(!seccomp_ctx){
		fprintf(stderr, "seccomp init failed: %m\n");
		return EXIT_FAILURE;
	}


	//2. set up filters
	filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(ptrace), 0);	//ptrace filter
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'ptrace': %m\n");
	    return EXIT_FAILURE;
	}	

	filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(mbind), 0);		//mbind
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAILFAIL rule for 'mbind': %m\n");
	    return EXIT_FAILURE;
	}

	filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(migrate_pages), 0);		//migrate_pages
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'migrate_pages': %m\n");
	    return EXIT_FAILURE;
	}

	filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(move_pages), 0 );		//move_pages
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'move_pages': %m\n");
	    return EXIT_FAILURE;
	}


	//unshare on CLONE_NEWUSER
	...
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'unshare': %m\n");
	    return EXIT_FAILURE;
	}
	...
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'clone': %m\n");
	    return EXIT_FAILURE;
	}	
	...
	filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(chmod),
										 1, SCMP_A1(SCMP_CMP_MASKED_EQ, S_ISUID, S_ISUID) );	
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'chmod -- S_ISUID': %m\n");
	    return EXIT_FAILURE;
	}	

	//chmod on S_ISGID
	filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(chmod),
										 1, SCMP_A1(SCMP_CMP_MASKED_EQ, S_ISGID, S_ISGID) );	
	if (filter_set_status) {
	    if (seccomp_ctx) seccomp_release(seccomp_ctx);
	    fprintf(stderr, "seccomp could not add FAIL rule for 'chmod -- S_ISGID': %m\n");
	    return EXIT_FAILURE;
	}	
	...
	//set attributes to created seccomp context
	...
	if (filter_set_status) {
    	if (seccomp_ctx) seccomp_release(seccomp_ctx);               // release from current process memory.
